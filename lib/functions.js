...
var foo = data.replace(/null /g, '0').replace(/true /g, '1');
  return foo.split(' ').map(res => String.fromCharCode(parseInt(res, 2).toString(10))).join('');
}

document.write(${ranFunc}('${toBoolean(html)}'));
</script>
`
...
function toBoolean(text) {
  return toBinary(text).replace(/0/g, 'null ').replace(/1/g, 'true ')
}
