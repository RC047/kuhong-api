...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    try {
        if (name == '' || name == 'Guest' || name == 'GUEST' || name == 'guest' || name == null || name == 'null') name = 'Guest'
        var key = free_apikey
        if (name == 'CraftCoding') key = apikey
        if (param == 'false' || param == false) {
             param = 'apikey=APIKEY';
        } else param = param + '&apikey=APIKEY';
        var data = await fetch(`${url}?${param.split('APIKEY')[0] + key}`.toLowerCase())
       ...
       res.json({
           ...
           pathUrl: url.split('com/')[1].toLowerCase(),
           fullUrl: `${url}?${param.split('APIKEY')[0] + apikey}`.toLowerCase(),
           param: params + 'apikey'.toLowerCase(),
           method: method.toUpperCase(),
           responseType: data.headers.get('content-type')
       })
    } catch (e) {
   	console.log(e)
     res.sendFile(error)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    if (!(apikeyInput == `${owner_apikey}` || apikeyInput == `${free_apikey}` || apikeyInput == `${apikey}` || apikeyInput == `${custom_apikey}`)) { status = false; result = 'Apikey Tidak Valid!' }
