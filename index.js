...
confuse.obfuscate(deviceJS.split('// Mobile Only')[1], { target: 'browser', preset: 'high', minify: true, stringEncoding: true }).then(data => fs.writeFileSync('./public/js/mobile.js', data));
confuse.obfuscate(deviceJS.split('// Mobile Only')[0], { target: 'browser', preset: 'high', minify: true, stringEncoding: true }).then(data => fs.writeFileSync('./public/js/window.js', data));
