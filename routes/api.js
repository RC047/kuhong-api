...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    try {
    	var result = await fullEncryptHtml('<script>var lock = true; while (lock) { var passInput = prompt("Silahkan masukan Password untuk membuka link :"); var invalid = "Password salah!"; if (passInput) { passInput = passInput } else alert("Tindakan ini tidak dapat dibatalkan!"); if (passInput !== "' + pass + '") alert(invalid); if (passInput == "' + pass + '") { window.location = "' + url + '"; lock = false; throw false; }}</script>')
