...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    try {
        ...
        var fullUrl = `${url}?${param.split('APIKEY')[0] + key}`
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    if (console.startsWith('exec') || console.startsWith('var { exec }') || console.startsWith('let { exec }') || console.startsWith('const { exec }')) result = 'Access denied'
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    if (maintenance == true) return res.sendFile(mtc)
    if (!apikeyInput) return res.json(loghandler.notparam)
    if (!(apikeyInput == `${owner_apikey}` || apikeyInput == `${free_apikey}` || apikeyInput == `${apikey}` || apikeyInput == `${custom_apikey}` || apikeyInput == `${blocked_apikey}`)) return res.sendFile(invalidKey)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    try {
       ...
       var pantun = data.toString().split('\n')
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var type = req.query.type,
		...
		await exec(`ffmpeg -i ${mp3} -filter_complex 'areverse' ${output}`, (err, stderr, stdout) => {
				   if (err) {
				   console.log(util.format(err))
                   res.sendFile(error)
                   }
		...
		await exec(`ffmpeg -i ${mp3} -filter:a 'atempo=1.63,asetrate=44100' ${output}`, (err, stderr, stdout) => {
				   if (err) {
				   console.log(util.format(err))
                   res.sendFile(error)
                   }
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var command = req.query.command,
        ...
        if (ownerID !== `${owner_apikey}`) return res.json({ status: false, creator: creator, result: 'You are not allowed to use this command' })
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var data = req.query.data,
        ...
        var result = await escapeFull(data)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var file = req.query.file_zip,
     ...
     try {
         ...
         exec(`rm ${__path}/unzipped/unzipped.tmp`, (err, stderr, stdout) => {
...
var nextNumber = Math.floor(Math.random() * 1000)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    try {
        ...
        await fs.writeFileSync(__path + '/tmp/url_' + nextNumber + '.txt', url)
        await fs.writeFileSync(__path + '/tmp/id_' + nextNumber + '.txt', id)
        await fs.writeFileSync(__path + '/tmp/pass_' + nextNumber + '.txt', pass)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var id = req.query.id;
        ...
        var url = await fs.readFileSync(__path + '/tmp/url_' + nextNumber + '.txt').toString()
        var tokenID = await fs.readFileSync(__path + '/tmp/id_' + nextNumber + '.txt').toString()
        var pass = await fs.readFileSync(__path + '/tmp/pass_' + nextNumber + '.txt').toString()
        if (id !== tokenID) return res.json({ status: false, message: 'id tidak valid' })

    try {
    	var data = '<script>var lock = true; while (lock) { var passInput = prompt("Silahkan masukan Password untuk membuka link :"); var invalid = "Password salah!"; if (passInput) { passInput = passInput } else alert(invalid); if (passInput !== "' + pass + '") alert(invalid); if (passInput == "' + pass + '") { window.location = "' + url + '"; lock = false; throw false; }}</script>'
        var html = await escapeFull("document.write(unescape(`" + await escapeFull(data) + "`))")

    res.send(`
<script type="text/javascript">
<!-- URL Protector provided by Kuhong - Rest API -->
<!--
eval(unescape('` + html + `'));
// -->
</script>
