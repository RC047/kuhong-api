...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var text = req.query.text,
        ...
        var result = await escapeFull(text)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var text = req.query.text,
        ...
        var result = await unescape(text)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    try {
        ...
        await fs.writeFileSync(__path + '/tmp/url_' + randomNumber + '.txt', url)
        await fs.writeFileSync(__path + '/tmp/id_' + randomNumber + '.txt', id)
        await fs.writeFileSync(__path + '/tmp/pass_' + randomNumber + '.txt', pass)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var id = req.query.id;
        ...
        var url = await fs.readFileSync(__path + '/tmp/url_' + randomNumber + '.txt').toString()
        var tokenID = await fs.readFileSync(__path + '/tmp/id_' + randomNumber + '.txt').toString()
        var pass = await fs.readFileSync(__path + '/tmp/pass_' + randomNumber + '.txt').toString()
        if (id !== tokenID) return res.json({ status: false, message: 'id tidak valid' })

    try {
    	var data = '<script>var lock = true; while (lock) { var passInput = prompt("URL TERKUNCI :\n\nSilahkan masukan Password untuk membuka link :"); var invalid = "Password salah!"; if (passInput == "") alert("Masukan Password!"); if (passInput) { passInput = passInput } else alert("Tindakan ini tidak dapat dibatalkan!"); if (passInput !== "' + pass + '") alert(invalid); if (passInput == "' + pass + '") { window.location = "' + url + '"; lock = false; throw false; }}</script>'
    	var result = await fullEncryptHtml(data, 'URL Locker provided by Kuhong - Rest API')
