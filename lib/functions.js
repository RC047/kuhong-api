...
var object = randomText()
var object2 = randomText()
var source = object + toBase64(toBase64(toBinary(reverseText(data))) + object2)
var result = `
eval(${ranFunc}('${source}'));

function ${ranFunc}(data) {
var tmp = atob(data).split('${object}')[1];
var tmp2 = atob(tmp.slice(0, tmp2.length - ${object2.length}));
var tmp3 = tmp2.trim().split(' ').map(v => String.fromCharCode(parseInt(v, 2))).join('');
var result = '';
for (var index = tmp3.length - 1; index >= 0; index--) result += tmp3[index]
