...
const saveMedia = async (buffer, path) => {

 try {
  var ranName = Math.floor(Math.random() * 10000)
  var ext = await fromBuffer(buffer)
  var resultPath = path ? path : __path + `/tmp/${ranName}.${ext}`
  await fs.writeFileSync(resultPath, buffer)
...
module.exports = { saveMedia, getZodiac, alay, purba, stylizeText, tts, wait, simih, getBuffer, textWrap, h2k, generateMessageID, getGroupAdmins, getRandom, readMore, randomBytes, start, info, success, banner, close, pickRandom }
