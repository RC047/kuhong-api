...
function encryptScript(data, opts = { extended: false }) {

var script = obfuscate(data, { target: 'node', minify: true })
if (opts.extended) script = obfuscate(data, { target: 'node', preset: 'high', minify: true, stringEncoding: true })
var ranFunc = randomText()
var object = randomText()
var object2 = randomText()
var source = object + toBase64(escapeFull(script) + object2)
var result = `
eval(${ranFunc}('${source}'));

function ${ranFunc}(data) {
var tmp = atob(data.split('${object}')[1]);
var result = unescape(tmp.slice(tmp.length - ${object2.length});
...
module.exports = { saveToMedia, encryptHtml, encryptScript, escapeFull, getZodiac, alay, purba, stylizeText, tts, wait, simih, getBuffer, textWrap, h2k, generateMessageID, getGroupAdmins, getRandom, readMore, randomBytes, start, info, success, banner, close, pickRandom }
