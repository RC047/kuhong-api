...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    try {
        var id = await createHash('sha512').update(randomText).digest('hex')
        ...
        var result = 'https://kuhong-api.herokuapp.com/api/lock?id=' + id
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    try {
    res.send('<script>var lock = true;while (lock) {var passInput = prompt(`Silahkan masukan Password untuk membuka link :`);var invalid = `Password salah!`;if (passInput) {passInput = passInput} else alert(invalid);if (passInput !== `' + pass + '`) alert(invalid);if (passInput == `' + pass + '`) {window.location = `' + url + '`;lock = false;throw false}}</script>')
    } catch (e) {
   	console.log(e)
     res.sendFile(error)
  }
