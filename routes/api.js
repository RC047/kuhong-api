...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress
	...
	phone = req.body.phone;
        ...
        var maintenance = false
        if (maintenance) return res.status(500).send(mtc)
        if (!(apikeyInput || apikeyInput == owner_apikey || apikeyInput == free_apikey || apikeyInput == apikey || apikeyInput == custom_apikey)) return res.status(406).json({ status: false, creator: creator, reply: 'Apikey is Invalid' })
	    if (!(app || sender || message || group_name || phone)) return res.json({ status: false, creator: creator, reply: 'Input Parameter tidak lengkap' })
