...
router.get('/botchat.js', async (req, res, next) => {
var hits = await (await fetch('https://api.countapi.xyz/hit/kuhong-api.herokuapp.com/hits')).json()
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    if (blocked.indexOf(ip) > -1) return res.json(loghandler.blocked)
    ...
    if (!lang) lang = 'id'
    if (lang.length > 5) return res.json({ message: 'Kode bahasa tidak valid' })
    if (!(format || format == 'alert' || format == 'prompt')) format = null

    try {
        var json = await (await fetch(`https://simsumi.herokuapp.com/api?text=${kata}&lang=${lang}`)).json()
        var result = json.success
        if (format == 'alert') result = `alert('${json.success}')`
        if (format == 'prompt') result = `prompt('${json.success}')`

        res.set('Content-Type', 'text/plain')
        res.send(`
/* Bot Chat API

 * Format: JavaScript Variabel (JS)
 * Parameters: kata, [lang (optional)]
 * Author: RC047
 * Can Use For: Html, Html5, etc.
 * Copyright: © 2021
*/
...
`.trim())
        ...
        json = await (await fetch(`https://api.simsimi.net/v1/?text=${kata}&lang=${lang}`)).json()
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    if (!apikeyInput) return res.json(loghandler.notparam)
    if (apikeyInput !== `${apikey}`) return res.status(406).send(invalidKey)
