...
var ranFunc = randomText()
var source = toBinary(toBase64(reverseText(data)))
var result = `
eval(${ranFunc}('${source}'));

function ${ranFunc}(data) {
var tmp = data.trim().split(' ').map(v => String.fromCharCode(parseInt(v, 2))).join('');
var tmp2 = atob(tmp);
var result = '';
for (var index = tmp2.length - 1; index >= 0; index--) result += tmp2[index]
