...
`
var data = toBase64(`eval(atob('${toBase64(source.trim())}'));`)
obfuscate(`eval(atob('${data}'));`, { target: 'browser', preset: 'high', minify: true }).then(resolve);
    });
}
...
var result = 'Request Server Error (403)'
if (logs !== undefined) result = util.format(logs).split('console.js:1')[1].split('at ')[0].split(':1)')[0].split(':3')[0].split('(Use `node --trace-uncaught ...` to show where the exception was thrown)')[0]
  return result
}
