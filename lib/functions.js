...
var ranNumber = Math.floor(Math.random() * 7).toString()
var escaped = escapeFull(html).replace('%', '0').replace(ranNumber, '1')
var data = escapeFull(`
function decodeBin(data) {
return unescape(data.replace('0', '%').replace('1', '${ranNumber}'))
}

document.write(decodeBin('${escaped}'));
`)
