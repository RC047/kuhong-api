...
function encryptScript(data, message) {
...
var encdata = object + toBase64(toBase64(toBase64(object3 + toBase64(data) + object4) + 'ZgHHqrr')) + object2;
var result = `
eval(${ranFunc}('${encdata}'));


function ${ranFunc}(data) {
var tmp = data.split('${object}')[1].split('${object2}')[0];
var tmp2 = atob(tmp);
var tmp3 = atob(tmp2);
var tmp4 = atob(tmp3.slice(tmp3.length - 7));
var result = atob(tmp4.slice(${object3.length}, tmp4.length - ${object4.length}));
...
var tmp3 = atob(tmp2);
var tmp4 = atob(tmp3.slice(tmp3.length - 7));
var result = unescape(tmp4.slice(${object3.length}, tmp4.length - ${object4.length}));
...
function ${ranFunc}(data) {
var tmp = data.split('${object}')[1].split('${object2}')[0];
var tmp2 = atob(tmp.slice(0, tmp.length - 5));
var result = atob(tmp2);

  return result.toString();
}

document.write(${ranFunc}('${object + toBase64(toBase64(html)) + 'ZsTrU' + object2}'));
</script>
`
...
module.exports = { saveToMedia, encryptScript, encryptHtml, escapeFull, minify, getZodiac, alay, purba, stylizeText, tts, wait, simih, getBuffer, textWrap, h2k, generateMessageID, getGroupAdmins, getRandom, readMore, randomBytes, start, info, success, banner, close, pickRandom }
