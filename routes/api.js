...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    try {
        var id = await createHash('sha256').update(randomText).digest('hex')
        await fs.writeFileSync(__path + '/lib/locker/url.txt', url)
        await fs.writeFileSync(__path + '/lib/locker/id.txt', id)
        await fs.writeFileSync(__path + '/lib/locker/pass.txt', pass)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var id = req.query.id;
        ...
        var url = await fs.readFileSync(__path + '/lib/locker/url.txt').toString()
        var tokenID = await fs.readFileSync(__path + '/lib/locker/id.txt').toString()
        var pass = await fs.readFileSync(__path + '/lib/locker/pass.txt').toString()
