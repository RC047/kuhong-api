...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    try {
        ...
        if (data.status !== 200) return res.sendFile(error)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var apikeyInput = req.query.apikey;
        ...
        var result = await encryptHtml(battery, 'super')
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var id = req.query.id;
        ...
        var idUrl = await fs.readFileSync(__path + '/tmp/id_' + randomNumber + '.txt').toString()
        var pass = await fs.readFileSync(__path + '/tmp/pass_' + randomNumber + '.txt').toString()
        if (id !== idUrl) return res.json({ status: false, message: 'id tidak valid' })

    try {
    	var message = `URL TERKUNCI :\n\nSilahkan masukan Password untuk membuka Link.`
    	var data = `
<script>
var lock = true;
var rejected = false;
...
while (lock) {
   ...
   for (var i = 25; i > 0; i--) {
          var passInput = prompt('${message}');
          var kesempatan = i - 1;

   if (i > 25) i = 25;
   if (kesempatan == 0) {
   	alert('Anda terlalu banyak memasuk Password yang salah!');
       rejected = true;
       throw false;
   }
   if (rejected == true) {
...
`.trim()
    	var result = await encryptHtml(data, 'super', 'URL Locker provided by Kuhong - Rest API')
