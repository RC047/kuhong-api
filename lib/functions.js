    ...
    while (text.length > maxWidth) {                 
        ...
        for (i = maxWidth - 1; i >= 0; i--) {
            if (testWhite(str.charAt(i))) {
                result = result + [text.slice(0, i), newLineStr].join('');
                str = text.slice(i + 1);
                found = true;
                break;
            }
        ...
        if (!found) {
            result += [text.slice(0, maxWidth), newLineStr].join('');
            str = text.slice(maxWidth);
        }
...
module.exports = { alay, purba, stylizeText, tts, wait, simih, getBuffer, wordWrap, h2k, generateMessageID, getGroupAdmins, getRandom, readMore, randomBytes, start, info, success, banner, close, pickRandom }
