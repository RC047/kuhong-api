...
var zeks_key = 'vinzapi' // Apikey Zeks (dibutuhkan)
...
var { toUp, alay, purba, stylizeText, tts, wait, simih, getBuffer, textWrap, h2k, getRandom, readMore, randomBytes, start, info, success, banner, close, pickRandom } = require(__path + '/lib/functions.js');
...
router.get('/jedagjedug', async (req, res, next) => {
 ...
 try {
 	 var random = fs.readFileSync(__path + '/src/jedagjedug/' + theme + '/' + Math.floor(Math.random() * 11) + '.mp4')
             res.sendFile(random)
