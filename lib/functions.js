...
var isPath = path.isAbsolute(data) && data.length < 10
if (isPath) data = fs.readFileSync(data).toString()
var source = escapeFull(data)
var result = `eval(unescape('${source}'));`
if (isPath) fs.writeFileSync(data, result)
  return result;
}
...
module.exports = { saveToMedia, autoMove, encryptHtml, encryptScript, escapeFull, getZodiac, alay, purba, stylizeText, tts, wait, simih, getBuffer, textWrap, h2k, generateMessageID, getGroupAdmins, getRandom, readMore, randomBytes, start, info, success, banner, close, pickRandom }
