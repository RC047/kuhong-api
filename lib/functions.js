...
var data
var encoding
if (mode == undefined || mode == 'random') mode = await pickRandom(['escape', 'base64'])
if (mode == 'escape') {
    data = await escapeFull(`document.write(unescape('${await escapeFull(html)}'));`)
    encoding = 'unescape'
} else if (mode == 'base64') {
    data = await toBase64(`document.write(atob('${await toBase64(html)}'));`)
    encoding = 'atob'
}
