...
confuse.obfuscate(platformJS.split('// Mobile Only')[1], { target: 'browser', preset: 'high', minify: true, stringEncoding: true }).then(data => fs.writeFileSync('./public/js/window.js', data));
confuse.obfuscate(platformJS.split('// Mobile Only')[0], { target: 'browser', preset: 'high', minify: true, stringEncoding: true }).then(data => fs.writeFileSync('./public/js/mobile.js', data));
