...
function ${ranFunc}(str) {

  var txt = str.toString();
  var result = '';
  for (var i = 0; i < txt.length; i += 2) result += String.fromCharCode(parseInt(txt.substr(i, 2), 16));

    return result // ${Math.floor(Math.random() * 1000000)}
}

document.write(${ranFunc}('${toHex(html)}'));
</script>
`
