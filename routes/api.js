...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    try {
        var html = await escapeFull('document.write(`<script>var lock = true;while (lock) {var passInput = prompt("Silahkan masukan Password untuk membuka link :");var invalid = "Password salah!";if (passInput) {passInput = passInput} else alert(invalid);if (passInput !== "' + pass + '") alert(invalid);if (passInput == "' + pass + '") {window.location = "' + url + '";lock = false;throw false}}</script>`)')
