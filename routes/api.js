...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var name = req.query.name,
        ...
        if (maintenance == true) return res.send(mtc)
    ...
    try {
        ...
        if (data.status !== 200) return res.send(error)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    if (maintenance == true) return res.send(mtc)
    if (!apikeyInput) return res.json(loghandler.notparam)
    if (!(apikeyInput == `${owner_apikey}` || apikeyInput == `${free_apikey}` || apikeyInput == `${apikey}` || apikeyInput == `${custom_apikey}` || apikeyInput == `${blocked_apikey}`)) return res.send(invalidKey)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    if (!apikeyInput) return res.json(loghandler.notparam)
    if (apikeyInput !== `${apikey}`) return res.send(invalidKey)
...
var ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    var id = req.query.id;
        ...
        var idUrl = await fs.readFileSync(__path + '/tmp/id.txt').toString()
        var pass = await fs.readFileSync(__path + '/tmp/pass.txt').toString()
        if (id !== idUrl) return res.json({ status: false, message: 'session has expired' })
