...
function autoMove(targetPath, name) {

try {
if (!targetPath.startsWith('/')) throw 'Error: Path must be a directory root files'
else if (!targetPath.endsWith('js')) throw 'Error: Only JavaScript (JS) Files are allowed to auto moved'
var ranName = randomText()
var pathName = `${__path}/public/js/${ranName}.js`
var data = fs.readFileSync(targetPath).toString()
fs.writeFileSync(pathName, data);
fs.writeFileSync(`${__path}/tmp/${name}_save.txt`, ranName + '.js')
...
var tmp = atob(data.split('${object}')[1].split('${object3}')[0]);
var tmp2 = atob(tmp.slice(0, tmp.length - ${object2.length}));
var result = atob(tmp2.replace(/${object4}/g, '').replace(/${object5}/g, ''));

  return result.toString();
}

document.write(${ranFunc}('${object + toBase64(toBase64(object4 + toBase64(html) + object5)) + object2 + object3}'));
</script>
`
...
function minify(data, callback) {
  return obfuscate(data, { target: 'node', minify: true }).then(res => callback(res));
}
...
module.exports = { saveToMedia, autoMove, encryptHtml, escapeFull, minify, getZodiac, alay, purba, stylizeText, tts, wait, simih, getBuffer, textWrap, h2k, generateMessageID, getGroupAdmins, getRandom, readMore, randomBytes, start, info, success, banner, close, pickRandom }
